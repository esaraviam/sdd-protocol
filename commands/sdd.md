...
5. **Await & reconcile (verify, don't trust).** A self-reported "done" is not enough. As each agent finishes, mark `"completed"` **only if all** of these pass; otherwise set it back to `"pending"`, clear the lock, and record the failure reason in the task JSON:
   - **Skill proof (falsifiable, not just present):** The report MUST include the marker `[SKILL-CONFIRMATION: <skill_name> | ... ]` exactly as defined in that skill's `SKILL.md`, AND the marker's file-bearing fields must be anchored to reality — not merely well-formed. Apply, in order:
     1. **Presence + name match:** marker exists and `<skill_name>` equals the task's `"skill"`. Missing or mismatched → reject.
     2. **Anchor cross-check:** every file path the marker names (e.g. `Components`, `Delivered Files`, `Files Reviewed`, `Files Audited`, `Implemented Files`) MUST appear in the task's `git diff`. A marker that names zero files while the diff is non-empty, or names a file absent from the diff, → reject. This is what makes the proof falsifiable: a free-form string an agent could invent without working will not survive the diff cross-check.
     3. **Verdict consistency:** if the marker carries a verdict/score field (`Verdict`, `Health Score`, `Safe to Merge`, `BLOCKING`), it must not contradict the Test/Criteria checks below (e.g. `Verdict: PASS` with a failing test command → reject).
     A marker that is present but fails (2) or (3) is treated as **no proof at all** — reject exactly as if it were missing.
